...
DEFAULT_PROFILE_PATH = settings.root_dir / "src" / "creator" / "creator_profile.json"
LEGACY_PROFILE_PATH = settings.root_dir / "creator_profiles" / "default_creator.json"
        ...
        "past_viral_content": [
            {
                ...
                "views": 28000,
                "hook_pattern": "before-after workflow",
            },
            {
                ...
                "views": 42000,
                "hook_pattern": "numbered utility list",
            },
        ],
...
def load_creator_profile(path: str | Path | None = None) -> dict[str, Any]:
    ...
    if not profile_path.exists():
        if path is None and LEGACY_PROFILE_PATH.exists():
            write_json(profile_path, read_json(LEGACY_PROFILE_PATH))
        else:
